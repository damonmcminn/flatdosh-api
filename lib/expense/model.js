...
function all(group) {
  return expenses.filter({ group: group }).eqJoin('email', r.table('users')).zip().withFields('name', 'amount', 'timestamp').orderBy(r.desc('timestamp')).run(conn).then(_db2['default'].toArray);
}
